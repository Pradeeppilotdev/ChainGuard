...
export const formatInvestigationReport = (
  input: InvestigatorInput,
  verdict: InvestigatorVerdict,
): string => {
  const evidence = verdict.evidence;
  const emoji = verdict.verdict === 'SAFE' ? '✅' : verdict.verdict === 'REVIEW' ? '⚠️' : '🚫';
  
  return `## 📋 Investigation Report

### Transaction Details
- **Sender UPI**: \`${input.transaction.sender_upi}\`
- **Receiver UPI**: \`${input.transaction.receiver_upi}\`
- **Amount**: \`₹${input.transaction.amount}\`

### Verdict
- **${emoji} ${verdict.verdict}** (Confidence: \`${(verdict.confidence * 100).toFixed(0)}%\`)

### Evidence Summary
- Account Age: \`${evidence.account_age_days}\` days
- Prior Flags: \`${evidence.prior_flags}\`
- Connected Flagged: \`${evidence.connected_flagged_accounts}\`
- Blacklisted: \`${evidence.is_blacklisted ? 'Yes' : 'No'}\`
- Analyst Score: \`${evidence.analyst_score}\`
- Amount Ratio: \`${evidence.amount_ratio.toFixed(2)}\`

### Triggered Rules
${evidence.triggered_rules.length > 0 
  ? evidence.triggered_rules.map(r => `- \`${r}\``).join('\n')
  : '- None'}
...
---
*Report generated by ChainGuard Investigator Agent*`;
};
...
OUTPUT FORMAT:
Use markdown formatting in your reasoning:
- Use \`code blocks\` for transaction IDs, UPI IDs, and numerical values
- Use **bold** for key findings and verdicts
- Use bullet lists for evidence summaries
- Use headers (##) to separate sections when detailed
